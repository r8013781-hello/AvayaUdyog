...
async function request(path, { method = "GET", body, auth = false } = {}) {
  ...
  if (!response.ok) {
    ...
    // Some endpoints (e.g. the Google review sync) send extra context the UI
    // can show — the raw provider message and which settings are missing.
    if (data.detail) error.detail = data.detail;
    if (data.missing) error.missing = data.missing;
...
export const api = {
  ...
  getReceipts: () => request("/receipts", { auth: true }),
  getReceipt: (id) => request(`/receipts/${id}`, { auth: true }),
  createReceipt: (receipt) => request("/receipts", { method: "POST", body: receipt, auth: true }),
  updateReceipt: (id, receipt) => request(`/receipts/${id}`, { method: "PATCH", body: receipt, auth: true }),
  setReceiptStatus: (id, status) => request(`/receipts/${id}/status`, { method: "PATCH", body: { status }, auth: true }),
  deleteReceipt: (id) => request(`/receipts/${id}`, { method: "DELETE", auth: true }),
...
export const RESOURCES = ["leads", "customers", "followups", "quotations", "projects", "receipts"];
